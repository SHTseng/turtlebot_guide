...
TEST(StateDetectTest, setRobotState)
{
  ros::NodeHandle nh("");

  ros::service::waitForService("/gazebo/get_model_state", 5.0);
  ...
  EXPECT_TRUE(inTolerance(get_model_state_srv.response.pose.position.x, 1.5, 0.1));
  ...
  ros::service::waitForService("/gazebo/set_model_state", 5.0);
